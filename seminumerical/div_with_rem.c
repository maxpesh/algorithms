...
/*
 * ∀a∈ℤ,b∈ℕ ∃!q,r∈ℤ: a=b×q+r
 * ⎧r∈[-(b-1)/2,(b-1)/2], b-odd
 * ⎨  
 * ⎩r∈(-b/2,b/2], b-even
 * q=⌈a/b-0.5⌉ 
 */
div_t div_nearest_int(int a, unsigned b)
{
	div_t dt;
	int bs;
	...
	if (b == 0 || b > INT_MAX)
		errx(EXIT_FAILURE, "The denominator (%u) is out of range. "
		    "Should be (0,%d]", b, INT_MAX);
	bs = (int)b;
	dt.quot = (int)ceil(((double)a / bs) - 0.5);
	dt.rem = a - bs * dt.quot;
	return dt;
}
...
static void test_div_nearest_int(void)
{
	...
	dt = div_nearest_int(8, 3);
	assert(dt.quot == 3);
	assert(dt.rem == -1);
	dt = div_nearest_int(-8, 3);
	assert(dt.quot == -3);
	assert(dt.rem == 1);
	dt = div_nearest_int(1, 2);
	assert(dt.quot == 0);
	assert(dt.rem == 1);
	dt = div_nearest_int(-1, 2);
	assert(dt.quot == -1);
	assert(dt.rem == 1);
	dt = div_nearest_int(0, 5);
	assert(dt.quot == 0);
	assert(dt.rem == 0);
}
